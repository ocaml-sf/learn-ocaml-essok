...
For convenience, we have a live API server running at https://essok.productionready.io/api for the application to make requests against. You can view [the API spec here](https://github.com/GoThinkster/productionready/blob/master/api) which contains all routes & responses for the server.
...
The example application is a social blogging site (i.e. a Medium.com clone) called "Essok". It uses a custom API for all requests, including authentication. You can view a live demo over at https://angular.realworld.io
...
- CRUD Servers
- CR*D Comments on servers (no updating required)
- GET and display paginated lists of servers
